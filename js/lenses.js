...
function loadLenses(isFocalMapView, isFullFrameEquivalent, onlyFujifilm) {
    $.getJSON("data/lenses.json", function (data) {
        if (onlyFujifilm) {
        ...
        if (isFocalMapView) {
            ...
            generateFocalMapView(data, isFullFrameEquivalent);
        } else {
            $("#lensDisplayArea").html(generateTableView(data, isFullFrameEquivalent));
        }
    });
}

// Function to generate the table view
function generateTableView(lenses, isFullFrameEquivalent) {
    var tableView =
        "<h2>Available Lens List</h2>" +
    ...
    lenses.forEach(function (lens, index) {
        var minFocalLength = isFullFrameEquivalent ? lens.MinFocalDistance * 1.5 : lens.MinFocalDistance;
        var maxFocalLength = isFullFrameEquivalent ? lens.MaxFocalDistance * 1.5 : lens.MaxFocalDistance;

        tableView +=
            "<tbody>" +
            '<tr data-toggle="collapse" data-target="#details-' +
            index +
            '">' + // '+' button targets the detail cell
            ...
            '<tr id="details-' +
            index +
            '" class="collapse">' + // detail row, initially collapsed
            '<td colspan="7">' + // detail row spans all columns
            "Linear Motor: " +
            (lens.LinearMotor ? "Yes" : "No") +
            "<br>" +
            "Image Stabilization: " +
            (lens.ImageStabilization ? "Yes" : "No") +
            "<br>" +
            "Aperture Ring: " +
...
function generateFocalMapView(lenses, isFullFrameEquivalent) {
    ...
    var chartArea = svg.append("g").attr("transform", "translate(" + chartMargin + "," + chartMargin + ")");

    // Create a scale for x-axis (Focal Length)
    var xScale = d3
        .scaleLog() // Use a logarithmic scale for x-axis
        .domain([6, 600]) // Focal length range 8mm to 600mm
